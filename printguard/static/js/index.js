...
const camPredictionTimeDisplay = document.getElementById(
  "camPredictionTimeDisplay",
);
const camTotalDetectionsDisplay = document.getElementById(
  "camTotalDetectionsDisplay",
);
const camFrameRateDisplay = document.getElementById("camFrameRateDisplay");
const camDetectionToggleButton = document.getElementById(
  "camDetectionToggleButton",
);
const camDetectionLiveIndicator =
  document.getElementsByClassName("live-indicator");
...
const settingsMajorityVoteThreshold = document.getElementById(
  "majority_vote_threshold",
);
const settingsMajorityVoteThresholdLabel = document.getElementById(
  "majority_vote_threshold_val",
);
const settingsMajorityVoteWindow = document.getElementById(
  "majority_vote_window",
);
const settingsMajorityVoteWindowLabel = document.getElementById(
  "majority_vote_window_val",
);
...
function updateSelectedCameraSettings(d) {
  ...
  if (
    !hasPrinter &&
    (settingsCountdownAction.value === "cancel_print" ||
      settingsCountdownAction.value === "pause_print")
  ) {
...
function updateSelectedCamerasPrinterModal(
  printerStatus,
  printerTemperature,
  printerBedTemperature,
) {
  ...
  const hasPrinter =
    currentCameraPrinterConfig !== null &&
    currentCameraPrinterConfig !== undefined;
...
function updateCameraSelectionListData(d) {
  cameraItems.forEach((item) => {
    ...
    if (cameraId == d.camera_uuid) {
      ...
      item.querySelector("#lastTimeValue").textContent = d.last_time
        ? new Date(d.last_time * 1000).toLocaleTimeString()
        : "-";
      item.querySelector(".camera-prediction").style.color =
        d.last_result === "success" ? "green" : "red";
      ...
      item.querySelector("#cameraPreview").src =
        `/camera/feed/${d.camera_uuid}`;
    }
  });
}
...
function removeCamera(cameraUUID) {
  ...
  fetch("/camera/remove", {
    ...
    .then((response) => {
      if (!response.ok) {
        return response.json().then((errData) => {
          throw new Error(
            `Failed to remove camera ${cameraUUID}: ${errData.detail || response.statusText}`,
          );
        });
      }
    ...
    .then(() => {
      const cameraItem = document.querySelector(
        `.camera-item[data-camera-id="${cameraUUID}"]`,
      );
      if (cameraItem) {
        cameraItem.remove();
      }
...
function updatePolledPrinterData(d) {
  ...
  updateSelectedCamerasPrinterModal(jobState, nozzleTemp, bedTemp);
}

function fetchAndUpdateMetricsForCamera(cameraUUID) {
  if (!cameraUUID) {
    console.warn(
      "Cannot fetch metrics: invalid camera UUID provided:",
      cameraUUID,
    );
    return;
  }
  fetch(`/camera/state`, {
    ...
    .then((response) => {
      if (!response.ok) {
        return response
          ...
          .then((errData) => {
            throw new Error(
              `Failed to fetch camera state for camera ${cameraUUID}: ${errData.detail || response.statusText}`,
            );
          })
          .catch(() => {
            throw new Error(
              `Failed to fetch camera state for camera ${cameraUUID}: ${response.statusText}`,
            );
          });
      }
      return response.json();
    ...
    .then((data) => {
      const metricsData = {
        ...
        total_detections: data.detection_times
          ? data.detection_times.length
          : 0,
    ...
    .catch((error) => {
      console.error(
        `Error fetching metrics for camera ${cameraUUID}:`,
        error.message,
      );
      const emptyMetrics = {
        camera_uuid: cameraUUID,
...
function sendDetectionRequest(isStart) {
  if (cameraUUID === null || cameraUUID === undefined) {
    console.warn(
      `Cannot ${isStart ? "start" : "stop"} detection: no valid camera selected`,
    );
    return;
  }
  fetch(`/detect/live/${isStart ? "start" : "stop"}`, {
    ...
    .then((response) => {
      if (response.ok) {
        ...
        response
          ...
          .then((errData) => {
            console.error(
              `Failed to ${isStart ? "start" : "stop"} live detection for camera ${cameraUUID}. Server: ${errData.detail || response.statusText}`,
            );
          })
          .catch(() => {
            console.error(
              `Failed to ${isStart ? "start" : "stop"} live detection for camera ${cameraUUID}. Status: ${response.status} ${response.statusText}`,
            );
          });
      }
    })
    .catch((error) => {
      console.error(
        `Network error or exception during ${isStart ? "start" : "stop"} request for camera ${cameraUUID}:`,
        error,
      );
    });
}
...
cameraItems.forEach((item) => {
  item.addEventListener("click", function () {
    ...
    if (cameraId) {
      const nickname = this.querySelector(
        ".camera-header span:first-child",
      ).textContent;
...
async function checkNotificationsEnabled() {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      console.error(
        "Failed to fetch notification status from server:",
        resp.status,
      );
      return false;
    }
...
notificationsBtn.addEventListener("click", async () => {
  ...
  try {
    if (await checkNotificationsEnabled()) {
      ...
      try {
        const res = await fetch("/notification/unsubscribe", {
          method: "POST",
        });
...
document
  .querySelectorAll('.settings-form input[type="range"]')
  .forEach((slider) => {
...
function saveFeedSettings() {
  const settings = {
    ...
    stream_tunnel_fps: parseInt(
      document.getElementById("streamTunnelFps").value,
    ),
    stream_jpeg_quality: parseInt(
      document.getElementById("streamJpegQuality").value,
    ),
    stream_max_width: parseInt(document.getElementById("streamMaxWidth").value),
    detections_per_second: parseInt(
      document.getElementById("detectionsPerSecond").value,
    ),
    detection_interval_ms: parseInt(
      document.getElementById("detectionInterval").value,
    ),
    printer_stat_polling_rate_ms: parseInt(
      document.getElementById("printerStatPollingRate").value,
    ),
    min_sse_dispatch_delay_ms: parseInt(
      document.getElementById("minSseDispatchDelay").value,
    ),
  };
...
function initializeFeedSettings() {
  loadFeedSettings().then(() => {
    document
      .querySelectorAll('.feed-setting-item input[type="range"]')
      .forEach((slider) => {
  ...
  return fetch("/get-feed-settings", {
    ...
    .then((data) => {
      if (data.success && data.settings) {
        ...
        updateSliderValue(
          "detectionsPerSecond",
          settings.detections_per_second,
        );
        updateSliderValue("detectionInterval", settings.detection_interval_ms);
        updateSliderValue(
          "printerStatPollingRate",
          settings.printer_stat_polling_rate_ms,
        );
        updateSliderValue(
          "minSseDispatchDelay",
          settings.min_sse_dispatch_delay_ms,
        );
      }
    })
    .catch((error) => {
...
function unlinkPrinter() {
  ...
  if (!camUUID) return;
  if (
    confirm("Are you sure you want to unlink this printer from the camera?")
  ) {
    stopPrinterStatusPolling();
...
document
  .getElementById("linkPrinterBtn")
  ?.addEventListener("click", openPrinterModal);
document
  ...
  fetch(`/camera/state`, {
    ...
    .then((res) => {
      if (!res.ok) {
        return res.json().then((errData) => {
          throw new Error(
            `Failed to fetch camera state for camera ${cameraUUID}: ${errData.detail || res.statusText}`,
          );
        });
      }
      return res.json();
    ...
    .then((data) => {
      ...
      if (data.printer_config) {
        ...
        document.getElementById("modalPrinterName").textContent =
          data.printer_config.name;
        document.getElementById("modalPrinterType").textContent =
          data.printer_config.printer_type +
          " | " +
          data.printer_config.base_url;
      } else {
...
document
  .getElementById("modalUnlinkPrinterBtn")
...
document
  .getElementById("modalPrinterConnectionType")
  .addEventListener("change", (e) => {
    const selectedType = e.target.value;
    document.getElementById("modalOctoprintConfig").style.display =
      selectedType === "octoprint" || selectedType === "moonraker"
        ? "block"
        : "none";
  });

document
  .getElementById("linkPrinterForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const submitButton = e.target.querySelector('button[type="submit"]');
    const originalButtonText = submitButton.textContent;
    const printerType = document
      .getElementById("modalPrinterConnectionType")
      .value.trim();
    const printerName = document
      .getElementById("modalPrinterNameInput")
      .value.trim();
    const baseUrl = document
      .getElementById("modalOctoprintUrlInput")
      .value.trim();
    const apiKey = document
      .getElementById("modalOctoprintApiKeyInput")
      .value.trim();
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Failed to link printer:", errorMessage);
        alert("Failed to link printer: " + errorMessage);
        return;
      }
      ...
      alert(
        "Error linking printer. Please check your connection and try again.",
      );
    } finally {
...
const snapshotPollIntervalInput = document.getElementById(
...
useSnapshotModeInput?.addEventListener("change", () => {
  if (snapshotIntervalGroup) {
    snapshotIntervalGroup.style.display = useSnapshotModeInput.checked
      ? "block"
      : "none";
  }
  if (enablePreview.checked) {
