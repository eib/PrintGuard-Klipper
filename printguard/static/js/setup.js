...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showSection(sectionId) {
    ...
    progressContainer.querySelectorAll(".progress-step").forEach((step) => {
      ...
      if (stepId === sectionId) {
        ...
        (stepId === "vapid" &&
          (setupState.vapidConfigured || setupState.vapidSkipped)) ||
        (stepId === "ssl" &&
          (setupState.sslConfigured || setupState.sslSkipped)) ||
        ...
        (stepId === "cloudflare-download" &&
          setupState.cloudflareDownloadConfigured)
      ) {
    ...
    if (sectionId === "finish") {
      ...
      if (selectedNetworkOption === "external") {
        ...
        const tunnelStatus = setupState.tunnelInitialized
          ? "Initialized ✓"
          : setupState.tunnelConfigured
        ...
        document.getElementById("summary-ssl-status").textContent =
          setupState.sslConfigured
            ? "Configured ✓"
            : setupState.sslSkipped
              ? "Skipped"
              : "Not Configured";
  ...
  document
    .getElementById("external-network-btn")
  ...
  document.getElementById("ngrok-btn").addEventListener("click", () => {
    ...
    document
      .querySelectorAll(".tunnel-option")
      .forEach((btn) => btn.classList.remove("selected"));
  ...
  document
    .getElementById("cloudflare-global-key")
  ...
  document
    .getElementById("save-tunnel-settings")
    .addEventListener("click", async () => {
      ...
      if (selectedTunnelProvider === "ngrok") {
        ...
        const isGlobalKey = document.getElementById(
  ...
  document
    .getElementById("generate-vapid-keys-btn")
    .addEventListener("click", async () => {
      try {
        const response = await fetch("/setup/generate-vapid-keys", {
          method: "POST",
        });
        if (response.ok) {
  ...
  document
    .getElementById("import-vapid-keys-btn")
    .addEventListener("click", () => {
      document.getElementById("vapid-form").style.display = "block";
    });

  document
    .getElementById("save-vapid-settings")
    .addEventListener("click", async () => {
      const publicKey = document
        .getElementById("vapid-public-key")
        .value.trim();
      const privateKey = document
        .getElementById("vapid-private-key")
        .value.trim();
      const subjectInput = document
        .getElementById("vapid-subject")
        .value.trim();
  ...
  document.getElementById("skip-vapid-setup").addEventListener("click", () => {
    setupState.vapidConfigured = false;
    setupState.vapidSkipped = true;
    setupState.vapidData = { skipped: true };
    if (
      selectedNetworkOption === "external" &&
      selectedTunnelProvider === "cloudflare"
    ) {
  ...
  document
    .getElementById("generate-ssl-cert-btn")
    .addEventListener("click", async () => {
      try {
        const response = await fetch("/setup/generate-ssl-cert", {
          method: "POST",
        });
        if (response.ok) {
          setupState.sslConfigured = true;
          setupState.sslSkipped = false;
  ...
  document
    .getElementById("import-ssl-cert-btn")
    .addEventListener("click", () => {
  ...
  document.getElementById("skip-ssl-setup").addEventListener("click", () => {
    setupState.sslConfigured = false;
    setupState.sslSkipped = true;
    setupState.sslData = { skipped: true };
    showSection("finish");
  });
  ...
  document
    ...
    .addEventListener("click", async () => {
      try {
        const startupMode =
          selectedNetworkOption === "external" ? "tunnel" : "local";
        ...
        if (response.ok) {
          alert(
            "Setup complete! To finalize, please restart the server. Redirecting to setup page...",
          );
  ...
  document
    .getElementById("skip-finish-review")
    .addEventListener("click", () => {
      document.getElementById("finish-setup-btn").click();
    });

  document
    .getElementById("continue-to-finish")
    .addEventListener("click", () => {
      if (selectedNetworkOption === "external") {
        showSection("vapid");
      } else {
  ...
  document
    .getElementById("retry-initialization")
    .addEventListener("click", () => {
      initializeNgrokTunnel();
    });

  document
    .getElementById("back-to-tunnel-config")
    .addEventListener("click", () => {
      showSection("tunnel");
    });
  ...
  async function initializeNgrokTunnel() {
    ...
    try {
      ...
      if (response.ok && result.success) {
        document.getElementById("initialization-loading").style.display =
          "none";
        document.getElementById("initialization-success").style.display =
          "block";
        document.getElementById("provider-name").textContent =
          result.provider || selectedTunnelProvider;
        document.getElementById("provider-url").textContent =
          result.url || "N/A";
  ...
  async function fetchCloudflareAccountsZones() {
    document.getElementById("cloudflare-selection-loading").style.display =
      "block";
    document.getElementById("cloudflare-selection-content").style.display =
      "none";
    document.getElementById("cloudflare-selection-error").style.display =
      "none";
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        document.getElementById("cloudflare-selection-error").style.display =
          "block";
        document.getElementById("cloudflare-error-message").textContent =
          result.detail || "Unknown error occurred";
      }
    } catch (error) {
  ...
  document
    .getElementById("cloudflare-zone-select")
    .addEventListener("change", (e) => {
  ...
  document
    .getElementById("configure-cloudflare-tunnel")
    .addEventListener("click", async () => {
      ...
      const subdomain = document
        .getElementById("cloudflare-subdomain")
        .value.trim();
      if (!accountId) {
      ...
      document.getElementById("cloudflare-selection-content").style.display =
        "none";
      document.getElementById("cloudflare-tunnel-loading").style.display =
        "block";
      try {
  ...
  document
    .getElementById("retry-cloudflare-fetch")
    .addEventListener("click", () => {
      fetchCloudflareAccountsZones();
    });

  document
    .getElementById("back-to-tunnel-settings")
    .addEventListener("click", () => {
      showSection("tunnel");
    });
  ...
  document
    .getElementById("continue-to-finish-from-cloudflare")
    .addEventListener("click", () => {
      showSection("finish");
    });
